...
#define CICLOS_1MS              1000.0/(1000000.0/(FOSC/TICKS))
#define DELAY_10_CICLOS         10
#define DELAY_100_CICLOS        100
#define CICLOS_LOOP_DELAY_MS    30  // PENDIENTE DE CONTAR.
...
// -------- Selecciona el la función de espara más apropiado ------
  ...
  #define CALL_DELAY_MS      delay_x10_cycles(CICLOS_1MS/11)
#elif (FOSC/TICKS) < 2000000
  #define CALL_DELAY_MS      delay_x10_cycles((CICLOS_1MS-CICLOS_LOOP_DELAY_MS)/DELAY_10_CICLOS)
#else
  #define CALL_DELAY_MS      delay_x100_cycles((CICLOS_1MS-CICLOS_LOOP_DELAY_MS)/DELAY_100_CICLOS)
#endif

void delay_x10_cycles(uint8_t x10cycles);
void delay_x100_cycles(uint8_t x100cycles);
void delay_ms(uint16_t delayTimeMS);

#pragma disable_warning 85  // Evita que se muestre aviso de no uso de variable de entrada de funciones.
#if (FOSC/TICKS) < 2000000
// ----------------------------------------------------------------
#pragma callee_saves delay_x10_cycles  // Llamada a función sin que el compilador salvaguarde datos.
void delay_x10_cycles(uint8_t x10cycles)
...
#else
// ----------------------------------------------------------------
#pragma callee_saves delay_x100_cycles  // Llamada a función sin que el compilador salvaguarde datos.
void delay_x100_cycles(uint8_t x100cycles)
{
                                                // 2 ciclos de llamada a función.
   __asm
...
// ----------------------------------------------------------------
#pragma callee_saves delay_ms  // Llamada a función sin que el compilador salvaguarde datos.
void delay_ms(uint16_t delayTimeMS)
{
//    while(delayTimeUS--)
//        CALL_DELAY_MS;
//  Para minimizar errores de tiempo insignificantes se traslada el bucle while a un
//  bloque ASM eliminando lineas de código innecesarias y que producian algunos ciclos de
//  espera superior a lo esperado.

// PENDIENTE DE CONTAR EL NUMERO DE CICLOS DE ESTE BUCLE = CICLOS_LOOP_DELAY_MS
// hay que sumar la carga del parámetro en el registro dptr y la llamada a la función delay_ms
//   mov    dptr,#0x01f4     //  1 ciclo
//   lcall	_delay_ms        //  2 ciclos
